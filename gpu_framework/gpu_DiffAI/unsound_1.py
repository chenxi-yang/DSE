...
def f_assign_max_z(x):
    return x.set_value(var(100000.0))
...
class Unsound_1(nn.Module):
    def __init__(self, l):
        ...
        self.ifelse_z = IfElse(target_idx=[1], test=self.bar, f_test=f_test, body=self.assign_min_z, orelse=self.assign_max_z)
    ...
    def forward(self, input, version="self_nn_learning"):
        if version == "self_nn_learning":
            # TODO: use a batch-wise way
            y = self.nn(input)
            x = torch.clone(y)
            x[y <= float(self.bar)] = 1.0
            x[y > float(self.bar)] = 100000.0
            res = x
        else:
