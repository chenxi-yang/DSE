...
<<<<<<< HEAD
    ...
    if 'Thermostat' in configs['benchmark']:
        safe_interval = [55.0, 83.0]
    if 'AC' in configs['benchmark']:
        safe_interval = [math.sqrt(40.0), 100000.0]
    if 'Racetrack' in configs['benchmark']:
        safe_interval = [0.5, 10000000.0]
        safe_intervals = [[4, 7], [4, 7], [4, 7], [4, 7],
                            [4, 8], [4, 8], [4, 8], [4, 8],
                            [4, 9], [4, 9], [4, 9], [4, 9],
                            [4, 10], [4, 10], [4, 10], 
                ...
                if 'Racetrack' in configs['benchmark'] and property_index == 0:
                    for y_idx, y_value in enumerate(y[1:]):
                        if y_value < safe_intervals[y_idx][0] or y_value > safe_intervals[y_idx][1]:
                            unsafe_trajectories_num += 1
                            break
                else:
                    if min(y[1:]) < safe_interval[0] or max(y[1:]) > safe_interval[1]:
                        unsafe_trajectories_num += 1
        else:
