...
class MountainCar1(nn.Module):
    def __init__(self, l, sig_range=10, nn_mode='all', module='linearrelu'):
        ...
        self.ifelse_acceleration = IfElse(target_idx=[3], test=seelf.u_p_var, f_test=f_test, body=self.assign_left_u, orelse=self.assign_right_u)
        ...
        self.program = nn.Sequential(
            self.while1,
            self.check_position,
            self.trajectory_update_1, # only use the final reward
        )
