...
class Program(nn.Module):
    ...
    def forward(self, input, version=None):
        if version == "single_nn_learning":
            y = self.nn(input)
            x = torch.clone(y)
            x[y <= float(self.bar)] = x[y <= float(self.bar)] # float(max_v) # -torch.square(x[y <= float(self.bar)]) - 1
            # x[y <= float(self.bar)] = x[y <= float(self.bar)]
            x[y > float(self.bar)] = float(min_v) # torch.square(x[y > float(self.bar)]) # float(min_v)
            res = x
        else:
