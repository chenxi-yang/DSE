...
def f_update_high_heat(x):
    return x.mul(var(15.0))

def f_update_low_heat(x):
...
class Program(nn.Module):
    def __init__(self, l, nn_mode='all'):
        ...
        self.h_unit_low = var(15.0)
        self.h_unit_high = var(10.0)
        self.steps = var(19)
        # balance temperature: 70.0

        self.nn_cool = LinearReLU(l=l)
        self.nn_heat_low = LinearReLU(l=l)
        self.nn_heat_high = LinearReLU(l=l)
        ...
        # assign to (h, isOn)
        self.assign_low_heat_nn = Assign(target_idx=[3, 2], arg_idx=[1], f=self.nn_heat_low)
        self.assign_update_low_heat = Assign(target_idx=[2], arg_idx=[2], f=f_update_low_heat)
        self.assign_warming = Assign(target_idx=[1], arg_idx=[1, 2], f=f_warming)
        self.low_heat_block = nn.Sequential(
            self.assign_low_heat_nn,
            self.assign_update_low_heat,
            self.assign_warming,
        )
        self.assign_high_heat_nn = Assign(target_idx=[3, 2], arg_idx=[1], f=self.nn_heat_high)
        self.assign_update_high_heat = Assign(target_idx=[2], arg_idx=[2], f=f_update_high_heat)
        self.assign_warming = Assign(target_idx=[1], arg_idx=[1, 2], f=f_warming)
        self.high_heat_block = nn.Sequential(
            self.assign_high_heat_nn,
            self.assign_update_high_heat,
            self.assign_warming,
        )

        self.ifelse_isOn_heat = IfElse(target_idx=[3], test=var(0.66), f_test=f_test, body=self.low_heat_block, orelse=self.high_heat_block)
        self.ifelse_isOn = IfElse(target_idx=[3], test=var(0.33), f_test=f_test, body=self.cool_block, orelse=self.ifelse_isOn_heat)
    ...
    def forward(self, input, version=None): # version describing data loss or safety loss
        if version == "single_nn_learning":
            ...
            heat_low_index = input[:, 1] == 1
            heat_high_index = input[:, 1] == 2
            res[cool_index] = self.nn_cool(input[cool_index][:, :-1])
            res[heat_low_index] = self.nn_heat_low(input[heat_low_index][:, :-1])
            res[heat_high_index] = self.nn_heat_high(input[heat_high_index][:, :-1])
        else:
