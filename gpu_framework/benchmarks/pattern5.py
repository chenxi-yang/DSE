...
def f_assign_max_z(x):
    return x.select_from_index(1, index0).add(max_v)

def f_assign_y(x):
    return x.select_from_index(1, index0).mul(a).add(b)
...
class Program(nn.Module):
    def __init__(self, l=1, nn_mode="simple"):
        ...
        self.assign_y = Assign(target_idx=[1], arg_idx=[0], f=f_assign_y)

        self.assign_min_z = Assign(target_idx=[2], arg_idx=[2], f=f_assign_min_z)
        self.assign_max_z = Assign(target_idx=[2], arg_idx=[0], f=self.nn)
