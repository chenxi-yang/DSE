...
def cal_safe_loss(theta, x, width):
    ...
    symbol_table_list = initialization(res_l, res_r, point_cloud)
    # ini_time = time.time()
    ...
    # run_time = time.time()
    # print(f"run time: {run_time - ini_time}")
    safe_loss = distance_f_interval(symbol_table_list, target)
    # print(f"------- SL end-----")
...
def gd_direct_noise(X_train, y_train, theta_l, theta_r, target, lambda_=lambda_, stop_val=0.01, epoch=1000, lr=0.00001, theta=None):
    print("--------------------------------------------------------------")
    print('----Gradient Direct Noise Descent Train Exp Case DSE----')
