...
def best_theta(X_train, y_train, lambda_):
    theta, loss, loss_list, q, c, time_out = optimize_f(X_train, y_train, theta_l, theta_r, target, lambda_=lambda_, stop_val=stop_val, epoch=num_epoch, lr=lr)
        ...
        for data_for_sample in data_for_sample_list:
            ...
            for i in range(5):
                ...
                for t in range(t_epoch):
                    ...
                    # BEST_theta(lambda)
                    theta, loss, loss_list, q, c, time_out = optimize_f(X_train, y_train, theta_l, theta_r, target, lambda_=new_lambda, stop_val=stop_val, epoch=num_epoch, lr=lr)
