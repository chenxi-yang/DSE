...
def update_res_in_branch(res_symbol_table, res, probability, counter, point_cloud):
...
def calculate_branch(target_idx, test, symbol_table, b):
    ...
    res_symbol_table = update_res_in_branch(res_symbol_table, res, probability, counter, point_cloud)

    return res_symbol_table
            

def calculate_branch_list(target_idx, test, symbol_table_list, b):
    res_list = list()
    for symbol_table in symbol_table_list: # for each element, split it. # c, delta
        res_symbol_table = calculate_branch(target_idx, test, symbol_table, b)
...
def sample(symbol_table_list, cur_sample_size=0):
    sample_time = time.time()
    length_before = len(symbol_table_list)

    # just for pofiling
    if length_before > constants.SAMPLE_SIZE:
        print(f"sampling time: {time.time() - sample_time}, length before: {length_before}")
        return symbol_table_list, 1.0
    ...
    # to_get_sample_size = min(len(symbol_table_list), constants.SAMPLE_SIZE) # new
    ...
    if to_get_sample_size > 0:
        res_symbol_table_list = random.choices(symbol_table_list, weights=[symbol_table['explore_probability'].data.item() for symbol_table in symbol_table_list], k=to_get_sample_size)
    ...
    return res_symbol_table_list, 1.0
...
class IfElse(nn.Module):
    ...
    def forward(self, x_list, cur_sample_size=0):
        ...
        body_list = calculate_branch_list(self.target_idx, test, x_list, '<')
        ...
        res_symbol_table_list, cur_sample_size = sample(res_symbol_table_list)
...
class While(nn.Module):
    ...
    def forward(self, symbol_table_list, cur_sample_size=0):
        ...
        while(len(symbol_table_list) > 0):
            body_list = calculate_branch_list(self.target_idx, self.test, symbol_table_list, '<')
            orelse_list= calculate_branch_list(self.target_idx, self.test, symbol_table_list, '>')
            ...
            if len(orelse_list) > 0:
                ...
                tmp_res_symbol_table_list, cur_sample_size = sample(tmp_res_symbol_table_list)
