...
class Interval:
    ...
    def getDelta(self):
        return (self.right.sub_l(self.left)).div(2.0)
...
class Box():
    ...
    def getInterval(self):
        return Interval.setInterval(c.add(delta), c.sub(delta))
    ...
    def sigmoid(self): # monotone function
        tp = F.sigmoid(self.c + self.delta)
        bt = F.sigmoid(self.c - self.delta)
