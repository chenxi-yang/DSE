...
def initialization_nn(x, width):
    data_list = list()
    data = domain.Box(var_list([0.0, 0.0, x[0], x[0]]), var_list([0.0, 0.0, width, width]))
...
def initialization_point_nn(x):
    data_list = list()
    data = domain.Box(var_list([0.0, 0.0, x[0], x[0]]), var_list([0.0] * 4))
    data_list.append(data)
    return data_list
...
#     def __init__(self, target_idx, arg_idx, l):
#         super().__init__()
#         self.linear1 = Linear(in_channels=2, out_channels=l)
#         self.linear2 = Linear(in_channels=l, out_channels=1)
#         self.sigmoid = Sigmoid()
#         self.target_idx = target_idx
#         self.arg_idx = torch.tensor(arg_idx) # select the element for computation

#     def forward(self, x_list):
#         res_list = list()
#         for idx, x in enumerate(x_list):
#             input = torch.index_select(x, 0, self.target_idx)
...
class ThermostatNN(nn.Module):
    def __init__(self, l):
        ...
        self.while1 = While(target_idx=[0], test=var(1.0), body=self.assign1)
