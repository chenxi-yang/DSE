...
if MODE == 2:
    ...
    def initialization(x_l, x_r):
        ...
        symbol_table['x1'] = domain.Interval(10.0, 10.0)
        symbol_table['y1'] = domain.Interval(0.0, 0.0)
        symbol_table['x2'] = domain.Interval(0.0, 0.0)
        symbol_table['y2'] = domain.Interval(10.0, 10.0)
...
def initialization_point(x):
    ...
    symbol_table['x1'] = var(10.0)
    symbol_table['y1'] = var(0.0)
    symbol_table['x2'] = var(0.0)
    symbol_table['y2'] = var(10.0)
...
def f_cal_dist(x):
    return torch.abs(x[1].sub(x[3])).add(torch.abs(x[2].sub(x[4])))
def f_udpate_dist(x):
    return torch.min(x[0], torch.abs(x[1].sub(x[3])).add(torch.abs(x[2].sub(x[4]))))
def f4(x):
    return var(1.5)
def f5(x):
    return var(0.0)
...
def f20(x):
    return var(3.5)
def f_sub(x):
    return torch.abs(x[0].sub(x[1]))
def f_add(x):
    return torch.abs(x[0].add(x[1]))
def f_add_one(x):
...
def construct_syntax_tree(Theta):
    ...
    l3 = Ifelse('dist', var(9.0), l4, l7, None)

    l2_1 = Assign(['dist', 'x1', 'y1', 'x2', 'y2'], f_cal_dist, l3) # cal dist(safe property) # TODO:use all left/right
    l2_0 = Assign(['x2', 'v2'], f_add, l2_1)
    ...
    l18 = Ifelse('steps', var(10.1), l19, l20, None)
    ...
    l27 = Assign(['x', 'x1', 'y1', 'x2', 'y2'], f_udpate_dist, l28)

    l1 = Ifelse('stage', var(1.0), l2, l8, l27)
...
def construct_syntax_tree_point(Theta):
    ...
    l3 = IfelsePoint('dist', var(9.0), l4, l7, None)

    l2_1 = AssignPoint(['dist', 'x1', 'y1', 'x2', 'y2'], f_cal_dist, l3) # cal dist(safe property) # TODO:use all left/right
    l2_0 = AssignPoint(['x2', 'v2'], f_add, l2_1)
    ...
    l18 = IfelsePoint('steps', var(10.1), l19, l20, None)
    ...
    l27 = AssignPoint(['x', 'x1', 'y1', 'x2', 'y2'], f_udpate_dist, l28)

    l1 = IfelsePoint('stage', var(1.0), l2, l8, l27)
...
def construct_syntax_tree_smooth_point(Theta):
    ...
    l3 = IfelsePointSmooth('dist', var(9.0), l4, l7, None)

    l2_1 = AssignPointSmooth(['dist', 'x1', 'y1', 'x2', 'y2'], f_cal_dist, l3) # cal dist(safe property) # TODO:use all left/right
    l2_0 = AssignPointSmooth(['x2', 'v2'], f_add, l2_1)
    ...
    l18 = IfelsePointSmooth('steps', var(10.1), l19, l20, None)
    ...
    l27 = AssignPointSmooth(['x', 'x1', 'y1', 'x2', 'y2'], f_udpate_dist, l28)

    l1 = IfelsePointSmooth('stage', var(1.0), l2, l8, l27)
