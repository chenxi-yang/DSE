...
def pho(X, intersection):
    partial_intersection_value = intersection.getVolumn().div(X.getVolumn().mul(f_beta(PARTIAL_BETA)))
...
#TODO: something to be optimized
def divide_list(l, k):
    avg = len(l) / float(k)
    res = list()
    last = 0.0
...
class WhileSimple:
    ...
    def execute(self, symbol_table_list):
        ...
        body_symbol_table_list[0]['probability'] = var(1.0)
        ...
        while(true_flag > 0):
            ...
            tmp_body_symbol_table_list = update_symbol_table_list_with_constraint(self.target, self.test, body_symbol_table_list, '<')
            if len(tmp_body_symbol_table_list) > 0:
                body_symbol_table_list = tmp_body_symbol_table_list
            else:
                true_flag = 0
                res_symbol_table_list = body_symbol_table_list
