...
# Electronic Oscillator deep branch
...
def initialization_smooth_point(x, y):
    if DOMAIN == "interval":
        ...
        symbol_table = dict()
        symbol_table['x1'] = domain.Interval(var(x[0]),  var(x[0]))
        symbol_table['y'] = domain.Interval(var(x[1]),  var(x[1]))
        symbol_table['z'] = domain.Interval(var(x[2]),  var(x[2]))
        symbol_table['omega1'] = domain.Interval(var(x[3]),  var(x[3]))
        symbol_table['omega2'] = domain.Interval(var(x[4]),  var(x[4]))
        symbol_table['tau'] = domain.Interval(var(x[5]),  var(x[5]))
