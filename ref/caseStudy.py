...
#NB: This function assumes a max planner speed of 0.5
#    and a time step of 0.1
def getCSPlan_old(dt, steps, maxD):
    ...
    if dist < 1:
        ...
        return np.array([-0.25, 0, 0, 0, 0, 0])
    return np.array([-0.25, -0.25, 0, 0, 0, 0])

#NB: This function assumes a max planner speed of 0.25
#    and a time step of 0.2
def getCSPlan(steps):
    ...
    if steps <= 9:
        ...
        return np.array([0, 0.25, 0, 0, 0, 0])
    return np.array([-0.25, 0.25, 0, 0, 0, 0])
...
states = np.array([0.04, 0.04, 0.0, 0.0, 0.0, 0])
planner = np.array([0, 0, 0, 0, 0, 0])
...
#model = models.load_model('./networks/tanh32x32_-0.4.h5')
...
allX.append(states[0])
allY.append(states[1])
...
for e in range(episodes):
    ...
    #cur_plan = getCSPlan(dtt, e, maxD)
    cur_plan = getCSPlan(e)
    ...
    allX.append(round(states[0], 3))
    allY.append(round(states[1], 3))
